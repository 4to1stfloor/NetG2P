...
##########################################################################################################################
getwd()
luad_path ="/home/seokwon/01.externaldata/luad/"
CancerType_in = "sclc_ucologne_2015"
luad_indi_path = paste0(luad_path, CancerType_in,"/")

dim(mut.count.mat_luad)
# expretion matrix and convert form RPKM to TPM
luad_rpkm = read.table(paste0(luad_indi_path, "data_mrna_seq_rpkm.txt" ), header = T)
luad_rpkm_filt = luad_rpkm
while (sum(duplicated(luad_rpkm_filt$Hugo_Symbol)) != 0 ) {
  print(sum(duplicated(luad_rpkm_filt$Hugo_Symbol)))
  dupl = which(duplicated(luad_rpkm_filt$Hugo_Symbol))[1]
  dupl_gene = luad_rpkm_filt[dupl,]$Hugo_Symbol
  tmp.df = luad_rpkm_filt[which(luad_rpkm_filt$Hugo_Symbol == dupl_gene),]
  tmp.df.2 = tmp.df[,-1]
  ...
  tmp.df.2= tmp.df.2 %>% relocate("Hugo_Symbol", .before = colnames(luad_rpkm_filt)[1])
  # tmp.df.2= tmp.df.2 %>% relocate("Entrez_Gene_Id", .before = "CH79T")
  luad_rpkm_filt = luad_rpkm_filt[-which(luad_rpkm_filt$Hugo_Symbol == dupl_gene),]
  luad_rpkm_filt = rbind(luad_rpkm_filt,tmp.df.2)
  tmp.df.2 = NULL
  tmp.df =NULL
}
...
luad_rpkm_filt_tmp =luad_rpkm_filt[,-1]
luad_rpkm_filt_tmp = luad_rpkm_filt_tmp/2
fpkmToTpm <- function(fpkm)
{
  exp(log(fpkm) - log(sum(fpkm)) + log(1e6))
}

luad_tpm_filt =sapply(luad_rpkm_filt_tmp, fpkmToTpm)  
luad_tpm_filt = as.data.frame(luad_tpm_filt)
luad_tpm_filt$Hugo_Symbol = luad_rpkm_filt$Hugo_Symbol
# luad_tpm_filt$Entrez_Gene_Id = luad_rpkm_filt$Entrez_Gene_Id
luad_tpm_filt= luad_tpm_filt %>% relocate("Hugo_Symbol", .before =  colnames(luad_tpm_filt)[1])
# eurprad_tpm_filt= eurprad_tpm_filt %>% relocate("Entrez_Gene_Id", .before = "CH79T")
rownames(luad_tpm_filt) = luad_tpm_filt$Hugo_Symbol
luad_tpm_filt = luad_tpm_filt[,-1]

# mutation count matrix
luad_maf.data = read.maf(paste0(luad_indi_path, "data_mutations.txt"))
mut.count.mat_luad = mutCountMatrix(luad_maf.data) 


saveRDS(luad_tpm_filt, file=paste0(luad_indi_path, CancerType_in ,"_exp_TPM_mat_filt_log_gene_symbol.rds")) #60466 436 
saveRDS(mut.count.mat_luad, file=paste0(luad_indi_path , CancerType_in ,"_mut_count_filt_data.rds"))
...
main.path = paste0(luad_path,CancerType_in, "/")
...
savepath = paste0(rdpn_main.path,"luad/", CancerType_in , "/03.RDPN/RDPN_",c(num.edge.multi),"X/") #mine RDPN ppi backbone based on the new ppi backbone 

# create folder
dir.create(paste0(main.path,"03.RDPN/"))
dir.create(paste0(main.path,"03.RDPN/RDPN_",c(num.edge.multi),"X/"))
dir.create(paste0(rdpn_main.path,"luad/", CancerType_in,"/03.RDPN"))
dir.create(paste0(rdpn_main.path,"luad/", CancerType_in, "/03.RDPN/RDPN_",c(num.edge.multi),"X/"))
...
ck.rdpn.file = list.files(path =  paste0(rdpn_main.path, "luad/", CancerType_in, "/03.RDPN/RDPN_",c(num.edge.multi),"X/"))

if (length(ck.file) == 1) {
  print("RDPN finish")
  next
  # print("1111")
} else if (dir.exists(paste0(rdpn_main.path, "luad/", CancerType_in, "/03.RDPN/RDPN_",c(num.edge.multi),"X/")) && length(ck.rdpn.file) != 150) {
  print("it stopped")
...
cal.surv.p <- function(CancerType, ppi.date, path.dat, level, filt.genes.n) { 
  ...
  cli_surv = cli_surv[!is.na(cli_surv$`Overall Survival Status`),]
  ...
  for (p in 1:as.numeric(dim(hyper.filt)[1])) {
    ...
    hyper.dat = as.data.frame(cbind(id = names(hyper.each), pvals= as.numeric(hyper.each)))
    #hyper.dat$group = "" 
    hyper.dat['group'] = ""
...
if (sum(is.na(pval.mat)) != 0) {
  pval.mat = pval.mat[,-which(colSums(is.na(pval.mat)) != 0)]
}
...
df.phyper.merge_log_all$Alive = ifelse(df.phyper.merge_log_all$vital_status == "Alive",
                                       1,0)
df.phyper.merge_log_all$Dead = ifelse(df.phyper.merge_log_all$vital_status == "Dead",
                                      1,0)
df.phyper.merge_log_all3 = df.phyper.merge_log_all[,c(-1,-2,-3,-4,-5,-6,-7)]
df.phyper.merge_log_all3
df.phyper.merge_vital_log_all2= df.phyper.merge_log_all3
df.phyper.merge_vital_log_all2
...
cli_sclc_uco_surv$status = NA
cli_sclc_uco_surv$status[which(cli_sclc_uco_surv$`Overall Survival Status` == "1:DECEASED")] = 1
cli_sclc_uco_surv$status[which(cli_sclc_uco_surv$`Overall Survival Status` == "0:LIVING")] = 0
...
df.phyper.merge_in_log_all_filt$Alive = ifelse(df.phyper.merge_in_log_all_filt$`Overall Survival Status` == "0:LIVING",
                                               1,0)
df.phyper.merge_in_log_all_filt$Dead = ifelse(df.phyper.merge_in_log_all_filt$`Overall Survival Status` == "1:DECEASED",
                                              1,0)
df.phyper.merge_log_in3 = df.phyper.merge_in_log_all_filt[,c(-1,-2,-3,-4,-5,-6)]
...
cli_lu_log_each_neural <- neuralnet( Alive + Dead~., data=df.phyper.merge_vital_log_all2[train_val,], hidden=hidden, err.fct = "sse" ,linear.output=TRUE, stepmax = 9000000)
...
cor(predicted_train_result[,1] , df.phyper.merge_vital_log_all2[train_val,]$Alive )
cor(predicted_train_result[,2] , df.phyper.merge_vital_log_all2[train_val,]$Dead )

results.cli_train_alive_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[train_val,]$Alive, prediction = predicted_train_result[,1])
results.cli_train_dead_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[train_val,]$Dead, prediction = predicted_train_result[,2])

#  (1) - Alive results

roundedresults.cli_alive_train_log = results.cli_train_alive_log_neural
roundedresults.cli_alive_train_log = ifelse(roundedresults.cli_alive_train_log >= 0.5 , 1 , 0) 
roundedresults.cli_alive_train_log = as.data.frame(roundedresults.cli_alive_train_log)
predicted_alive_train_neural_log = table(prediction = roundedresults.cli_alive_train_log$prediction, actual = df.phyper.merge_vital_log_all2[train_val,]$Alive)
...
#  (1) - Dead results
roundedresults.cli_dead_train_log = results.cli_train_dead_log_neural
roundedresults.cli_dead_train_log = ifelse(roundedresults.cli_dead_train_log >= 0.5 , 1 , 0) 
roundedresults.cli_dead_train_log = as.data.frame(roundedresults.cli_dead_train_log)
predicted_dead_train_neural_log = table(prediction = roundedresults.cli_dead_train_log$prediction, actual = df.phyper.merge_vital_log_all2[train_val,]$Dead)

acc_dead_train_neural = round((predicted_dead_train_neural_log[1,1] + predicted_dead_train_neural_log[2,2]) / sum(predicted_dead_train_neural_log) * 100, 2)
acc_dead_each_neural
...
cor(predicted_test_result[,1] , df.phyper.merge_vital_log_all2[test_val,]$Alive )
cor(predicted_test_result[,2] , df.phyper.merge_vital_log_all2[test_val,]$Dead )

results.cli_test_alive_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[test_val,]$Alive, prediction = predicted_test_result[,1])
results.cli_test_dead_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[test_val,]$Dead, prediction = predicted_test_result[,2])

#  (1) - Alive results

roundedresults.cli_alive_test_log = results.cli_test_alive_log_neural
roundedresults.cli_alive_test_log = ifelse(roundedresults.cli_alive_test_log >= 0.5 , 1 , 0) 
roundedresults.cli_alive_test_log = as.data.frame(roundedresults.cli_alive_test_log)
predicted_alive_test_neural_log = table(prediction = roundedresults.cli_alive_test_log$prediction, actual = df.phyper.merge_vital_log_all2[test_val,]$Alive)
...
#  (1) - Dead results
roundedresults.cli_dead_test_log = results.cli_test_dead_log_neural
roundedresults.cli_dead_test_log = ifelse(roundedresults.cli_dead_test_log >= 0.5 , 1 , 0) 
roundedresults.cli_dead_test_log = as.data.frame(roundedresults.cli_dead_test_log)
predicted_dead_test_neural_log = table(prediction = roundedresults.cli_dead_test_log$prediction, actual = df.phyper.merge_vital_log_all2[test_val,]$Dead)

acc_dead_test_neural = round((predicted_dead_test_neural_log[1,1] + predicted_dead_test_neural_log[2,2]) / sum(predicted_dead_test_neural_log) * 100, 2)
acc_dead_test_neural

# 3) validate with external data set
cli_sclc_log_in_neural.results <- neuralnet::compute(cli_lu_log_each_neural, df.phyper.merge_vital_log_in2)

predicted_in_result = cli_sclc_log_in_neural.results$net.result

cor(predicted_in_result[,1] , df.phyper.merge_vital_log_in2$Alive )
cor(predicted_in_result[,2] , df.phyper.merge_vital_log_in2$Dead )

results.cli_sclc_alive_in_log_neural <- data.frame(actual = df.phyper.merge_vital_log_in2$Alive, prediction = predicted_in_result[,1])
results.cli_sclc_dead_in_log_neural <- data.frame(actual = df.phyper.merge_vital_log_in2$Dead, prediction = predicted_in_result[,2])

#   (2) - Alive results
roundedresults.cli_alive_in_sclc_log = results.cli_sclc_alive_in_log_neural
roundedresults.cli_alive_in_sclc_log = ifelse(roundedresults.cli_alive_in_sclc_log >= 0.5 , 1 , 0) 
roundedresults.cli_alive_in_sclc_log = as.data.frame(roundedresults.cli_alive_in_sclc_log)

predicted_alive_in_sclc_neural_log = NULL
predicted_alive_in_sclc_neural_log = table(prediction = roundedresults.cli_alive_in_sclc_log$prediction, actual = df.phyper.merge_vital_log_in2$Alive)

acc_alive_in_sclc_neural = round((predicted_alive_in_sclc_neural_log[1,1] + predicted_alive_in_sclc_neural_log[2,2]) / sum(predicted_alive_in_sclc_neural_log) * 100, 2)
acc_alive_in_sclc_neural

#   (2) - Dead results
roundedresults.cli_dead_in_sclc_log = results.cli_sclc_dead_in_log_neural
roundedresults.cli_dead_in_sclc_log = ifelse(roundedresults.cli_dead_in_sclc_log >= 0.5 , 1 , 0) 
roundedresults.cli_dead_in_sclc_log = as.data.frame(roundedresults.cli_dead_in_sclc_log)

predicted_dead_in_sclc_neural_log = NULL
predicted_dead_in_sclc_neural_log = table(prediction = roundedresults.cli_dead_in_sclc_log$prediction, actual = df.phyper.merge_vital_log_in2$Dead)

acc_dead_in_sclc_neural = round((predicted_dead_in_sclc_neural_log[1,1] + predicted_dead_in_sclc_neural_log[2,2]) / sum(predicted_dead_in_sclc_neural_log) * 100, 2)
acc_dead_in_sclc_neural
...
pr_neural = ROCR::prediction(roundedresults.cli_dead_each_log$actual, roundedresults.cli_dead_each_log$prediction)
...
# (1) train by TCGA data
cli_lu_log_best_each_neural <- neuralnet( Alive + Dead~  P3P14 + P1P14 + P21P35 + P51P54 + P24P25 , data=df.phyper.merge_vital_log_all2[train_val,], hidden=hidden, err.fct = "sse" ,linear.output=TRUE, stepmax = 10000000)
...
cor(predicted_best_train_result[,1] , df.phyper.merge_vital_log_all2[train_val,]$Alive )
cor(predicted_best_train_result[,2] , df.phyper.merge_vital_log_all2[train_val,]$Dead )

results.cli_train_best_alive_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[train_val,]$Alive, prediction = predicted_best_train_result[,1])
results.cli_train_best_dead_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[train_val,]$Dead, prediction = predicted_best_train_result[,2])

#   A. Alive results

roundedresults.cli_best_alive_train_log = results.cli_train_best_alive_log_neural
roundedresults.cli_best_alive_train_log = ifelse(roundedresults.cli_best_alive_train_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_alive_train_log = as.data.frame(roundedresults.cli_best_alive_train_log)
predicted_best_alive_train_neural_log = table(prediction = roundedresults.cli_best_alive_train_log$prediction, actual = roundedresults.cli_best_alive_train_log$actual)
...
#   B. Dead results
roundedresults.cli_best_dead_train_log = results.cli_train_best_dead_log_neural
roundedresults.cli_best_dead_train_log = ifelse(roundedresults.cli_best_dead_train_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_dead_train_log = as.data.frame(roundedresults.cli_best_dead_train_log)
predicted_best_dead_train_neural_log = table(prediction = roundedresults.cli_best_dead_train_log$prediction, actual = roundedresults.cli_best_dead_train_log$actual)

acc_best_dead_train_neural = round((predicted_best_dead_train_neural_log[1,1] + predicted_best_dead_train_neural_log[2,2]) / sum(predicted_best_dead_train_neural_log) * 100, 2)
acc_best_dead_train_neural
...
cor(predicted_best_test_result[,1] , df.phyper.merge_vital_log_all2[test_val,]$Alive )
cor(predicted_best_test_result[,2] , df.phyper.merge_vital_log_all2[test_val,]$Dead )

results.cli_test_best_alive_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[test_val,]$Alive, prediction = predicted_best_test_result[,1])
results.cli_test_best_dead_log_neural <- data.frame(actual = df.phyper.merge_vital_log_all2[test_val,]$Dead, prediction = predicted_best_test_result[,2])

#   A. Alive results

roundedresults.cli_best_alive_test_log = results.cli_test_best_alive_log_neural
roundedresults.cli_best_alive_test_log = ifelse(roundedresults.cli_best_alive_test_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_alive_test_log = as.data.frame(roundedresults.cli_best_alive_test_log)
predicted_best_alive_test_neural_log = table(prediction = roundedresults.cli_best_alive_test_log$prediction, actual = roundedresults.cli_best_alive_test_log$actual)
...
#   B. Dead results
roundedresults.cli_best_dead_test_log = results.cli_test_best_dead_log_neural
roundedresults.cli_best_dead_test_log = ifelse(roundedresults.cli_best_dead_test_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_dead_test_log = as.data.frame(roundedresults.cli_best_dead_test_log)
predicted_best_dead_test_neural_log = table(prediction = roundedresults.cli_best_dead_test_log$prediction, actual = roundedresults.cli_best_dead_test_log$actual)

acc_best_dead_test_neural = round((predicted_best_dead_test_neural_log[1,1] + predicted_best_dead_test_neural_log[2,2]) / sum(predicted_best_dead_test_neural_log) * 100, 2)
acc_best_dead_test_neural
...
cor(predicted_best_in_result[,1] , df.phyper.merge_vital_log_in2$Alive )
cor(predicted_best_in_result[,2] , df.phyper.merge_vital_log_in2$Dead )

results.cli_sclc_best_alive_in_log_neural <- data.frame(actual = df.phyper.merge_vital_log_in2$Alive, prediction = predicted_best_in_result[,1])
results.cli_sclc_best_dead_in_log_neural <- data.frame(actual = df.phyper.merge_vital_log_in2$Dead, prediction = predicted_best_in_result[,2])

#   A. Alive results
roundedresults.cli_best_alive_in_sclc_log = results.cli_sclc_best_alive_in_log_neural
roundedresults.cli_best_alive_in_sclc_log = ifelse(roundedresults.cli_best_alive_in_sclc_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_alive_in_sclc_log = as.data.frame(roundedresults.cli_best_alive_in_sclc_log)

predicted_best_alive_in_sclc_neural_log = NULL
predicted_best_alive_in_sclc_neural_log = table(prediction = roundedresults.cli_best_alive_in_sclc_log$prediction, actual = df.phyper.merge_vital_log_in2$Alive)

acc_best_alive_in_sclc_neural = round((predicted_best_alive_in_sclc_neural_log[1,1] + predicted_best_alive_in_sclc_neural_log[2,2]) / sum(predicted_best_alive_in_sclc_neural_log) * 100, 2)
acc_best_alive_in_sclc_neural

#   B. Dead results
roundedresults.cli_best_dead_in_sclc_log = results.cli_sclc_best_dead_in_log_neural
roundedresults.cli_best_dead_in_sclc_log = ifelse(roundedresults.cli_best_dead_in_sclc_log >= 0.5 , 1 , 0) 
roundedresults.cli_best_dead_in_sclc_log = as.data.frame(roundedresults.cli_best_dead_in_sclc_log)

predicted_best_dead_in_sclc_neural_log = NULL
predicted_best_dead_in_sclc_neural_log = table(prediction = roundedresults.cli_best_dead_in_sclc_log$prediction, actual = roundedresults.cli_best_dead_in_sclc_log$actual)

acc_best_dead_in_sclc_neural = round((predicted_best_dead_in_sclc_neural_log[1,1] + predicted_best_dead_in_sclc_neural_log[2,2]) / sum(predicted_best_dead_in_sclc_neural_log) * 100, 2)
acc_best_dead_in_sclc_neural
...
merge_results = data.frame(acc_train = acc_alive_train_neural, acc_test = acc_alive_test_neural, acc_ex = acc_alive_in_sclc_neural,
                           acc_best_train = acc_best_alive_train_neural , acc_best_test = acc_best_alive_test_neural , acc_best_ex = acc_best_alive_in_sclc_neural,
                           hidden = hidden_char)


write.csv(merge_results, paste0(main.path ,"results_of_neuralnet_pathwaylink.csv"), row.names = CancerType_in)
