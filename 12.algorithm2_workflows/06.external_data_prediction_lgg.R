...
kindofCancer = "lgg" # you can change this
...
expression = fread(paste0(main.path,dir(path = main.path , pattern = "data_mrna")), header = T, fill=TRUE)
# expression = expression[,-2]
expression_wo_TCGA = select(expression, -contains("TCGA"))
# rectal_msk_2022 mrna_expression was normalized by DEseq2

expression_filt = expression_wo_TCGA
while (sum(duplicated(expression_filt$Hugo_Symbol)) != 0 ) {
  print(sum(duplicated(expression_filt$Hugo_Symbol)))
  dupl = which(duplicated(expression_filt$Hugo_Symbol))[1]
  dupl_gene = expression_filt[dupl,]$Hugo_Symbol
  tmp.df = expression_filt[which(expression_filt$Hugo_Symbol == dupl_gene),]
  tmp.df.2 = tmp.df[,-1]
  ...
  tmp.df.2= tmp.df.2 %>% relocate("Hugo_Symbol", .before = colnames(expression_filt)[1])
  # tmp.df.2= tmp.df.2 %>% relocate("Entrez_Gene_Id", .before = "CH79T")
  expression_filt = expression_filt[-which(expression_filt$Hugo_Symbol == dupl_gene),]
  expression_filt = rbind(expression_filt,tmp.df.2)
  tmp.df.2 = NULL
  tmp.df =NULL
}

tmp_rowname = expression_filt$Hugo_Symbol


# if paired-end

expression_filt = expression_filt[,-1]
expression_filt = expression_filt/2
...
expression_tpm_filt =sapply(expression_filt, fpkmToTpm)  
expression_tpm_filt = as.data.frame(expression_tpm_filt)
expression_tpm_filt$Hugo_Symbol = tmp_rowname
# luad_tpm_filt$Entrez_Gene_Id = luad_rpkm_filt$Entrez_Gene_Id
expression_tpm_filt= expression_tpm_filt %>% relocate("Hugo_Symbol", .before =  colnames(expression_tpm_filt)[1])
# eurprad_tpm_filt= eurprad_tpm_filt %>% relocate("Entrez_Gene_Id", .before = "CH79T")
expression_tpm_filt = as.data.frame(expression_tpm_filt)
rownames(expression_tpm_filt) = tmp_rowname
expression_tpm_filt = expression_tpm_filt[,-1]

if (sum(colSums(is.na(expression_tpm_filt)) != 0) != 0) {
  expression_tpm_filt = expression_tpm_filt[,-which(colSums(is.na(expression_tpm_filt)) != 0)]
}
expression_tpm_filt

# mutation count matrix
ex_maf.data = read.maf(paste0(main.path, "data_mutations.txt"))
mut.count.mat_ex = mutCountMatrix(ex_maf.data) 
mut.count.mat_ex = as.data.frame(mut.count.mat_ex)
mut.count.mat_ex_wo_TCGA = select(mut.count.mat_ex, -contains("TCGA"))


expression_filt_final= expression_tpm_filt[,intersect(colnames(mut.count.mat_ex_wo_TCGA) ,colnames(expression_tpm_filt ))]
mut.count.mat_ex_final = mut.count.mat_ex_wo_TCGA[,intersect(colnames(mut.count.mat_ex_wo_TCGA) ,colnames(expression_tpm_filt ))]

saveRDS(expression_filt_final, file=paste0(main.path,"difg_glass_2019_exp_TPM_mat_filt_log_gene_symbol.rds")) #60466 436 
saveRDS(mut.count.mat_ex_final, file=paste0(main.path,"difg_glass_2019_mut_count_filt_data.rds"))
...
savepath = paste0(rdpn_main.path,kindofCancer,"/", CancerType_in , "/03.RDPN/RDPN_",c(num.edge.multi),"X/") #mine RDPN ppi backbone based on the new ppi backbone 

# create folder
dir.create(paste0(main.path,"03.RDPN/"))
dir.create(paste0(main.path,"03.RDPN/RDPN_",c(num.edge.multi),"X/"))
dir.create(paste0(rdpn_main.path,kindofCancer,"/", CancerType_in , "/03.RDPN"))
dir.create(paste0(rdpn_main.path,kindofCancer,"/", CancerType_in , "/03.RDPN/RDPN_",c(num.edge.multi),"X/"))
...
ck.rdpn.file = list.files(path =  paste0(rdpn_main.path,kindofCancer,"/", CancerType_in , "/03.RDPN/RDPN_",c(num.edge.multi),"X/"))
...
if (length(ck.file) == 1) {
  ...
  saveRDS(pval.mat, file = paste0(result_savepath, "net_prop_pval_", random.num, "rep_total_samples_", ppi.date, ".rds"))
    ...
    for (i in 1:random.num) {
      ...
      df.rand.prop.score = as.data.frame(rand.prop.score)
      df.rand.prop.score[,colnames(rand.prop.score)[which(is.na( match(colnames(rand.prop.score), colnames(raw.prop.score))))]] =NULL
      idx.ID = match(colnames(df.rand.prop.score), colnames(raw.prop.score))
...
cal.surv.p <- function(CancerType, ppi.date, path.dat, level, filt.genes.n) { 
  ...
  mut.mat = readRDS(paste0(main.path, CancerType_in ,'_mut_count_filt_data.rds'))#16131 985
  ...
  cli_surv = cli_surv[!is.na(cli_surv$status),]
  ...
  cli_surv$submitter_id = cli_surv$`Sample ID`
  ...
  rownames(pval.com) = rownames(hyper.filt)[-wrong_cluster]
  colnames(pval.com) = gsub(".*\\.", "", CancerType_in)
  print(pval.com)
  if (level != "") {
