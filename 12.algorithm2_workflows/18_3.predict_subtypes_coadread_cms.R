...
sce_exp = tcga.calc.zscore(sce = sce, cms_genes$SYMBOL)
...
png(filename = paste0(CancerType,"_exp_cluster_complex_CMS.png"),
    width = 25, height = 25,  units = "cm" ,pointsize = 12,
    bg = "white", res = 1200, family = "")
tmp2 = ComplexHeatmap::pheatmap(as.matrix(t(sce_exp_df[,-ncol(sce_exp_df)])),
...
mut_count[cms_genes$SYMBOL,]
mut_count_filtered = mut_count[which(rownames(mut_count) %in% cms_genes$SYMBOL),]
nrow(mut_count_filtered)
...
if (all.equal(rownames(cms_filtered) , rownames(mut_count_filtered_tdf))) {
...
png(filename = paste0(CancerType,"_mut_cluster_complex_CMS.png"),
    width = 25, height = 25,  units = "cm" ,pointsize = 12,
    bg = "white", res = 1200, family = "")
tmp2 = ComplexHeatmap::pheatmap(as.matrix(t(mut_count_filtered_tdf_common[,-ncol(mut_count_filtered_tdf_common)])),
                                column_split = annotation_df$CMS,
                                annotation_col = annotation_df,
                                annotation_colors = list(CMS = CMS_colors),
                                cluster_cols = T)
