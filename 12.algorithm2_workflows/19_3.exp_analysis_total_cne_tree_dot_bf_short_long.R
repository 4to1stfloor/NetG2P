...
for (num_CancerType in Cancerlist) {
  ...
  } else if (all.equal(rownames(bf_zs_shared_exp_filt_df[rownames(bf_short_long),]) ,rownames(bf_short_long))) {
    bf_zs_shared_exp_filt_df$cluster = bf_short_long$cluster
  } 
  ...
  fig_path = paste0(filepath,"04.Results/short_long/analysis_gene_from_features/",CancerType,"_analysis_exp_bf/")
