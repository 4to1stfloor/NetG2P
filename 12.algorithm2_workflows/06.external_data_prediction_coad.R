...
##########################################################################################################################
getwd()
coad_path ="/home/seokwon/01.externaldata/coad/"
CancerType_in = "rectal_msk_2022"
coad_indi_path = paste0(coad_path, CancerType_in,"/")

# expretion matrix and convert form RPKM to TPM
coad_rpkm = read.table(paste0(coad_indi_path, "data_mrna_seq_expression.txt" ), header = T)
...
coad_maf.data = read.maf(paste0(coad_indi_path, "data_mutations.txt"))
...
if (length(ck.file) == 1) {
  ...
  for (i in random.num) {  # when there are 50 iterations 
    if (length(ck.rdpn.file) == 150) {
      next
    }
    g.rdpn = readRDS(paste0(sourcepath,"/ppi_backbone_",ppi.date,"_RDPN",c(i),".rds")) #13064 614246
