...
for (num_CancerType in Cancerlist) {
  ...
  # unique(anticancer_drug_filted$Product)
  # drugid = "Dactolisib"
  for (drugid in unique(gdsc_w_cluster$DRUG_ID)) {
    ...
    if (nrow(long_drug) > 1 & nrow(short_drug) > 1) {
      ...
      if (pval_drug$p.value < 0.05 & mean(long_drug$Z_SCORE) < mean(short_drug$Z_SCORE)) {
        tmp_drug$tolerant = "short"
  ...
  # total_drug_screen = total_drug_screen %>% filter(!is.na(PUTATIVE_TARGET))
  
  drug_target_features = data.frame()
  # dn = "Gemcitabine"
  for (dn in unique(total_drug_screen$DRUG_NAME)) {
    if (sum(anticancer_drug_filted$Product == dn) == 0) {
      next
    } else {
      tmp_anti = anticancer_drug_filted %>% filter(Product == dn)
      unique(tmp_anti$Indications)
      tmp_drug = total_drug_screen %>% filter(DRUG_NAME == dn)
      # target_genes = tmp_drug %>% pull(PUTATIVE_TARGET) %>% unique() %>% gsub(" \\([^\\)]+\\)","", . ) %>% str_split(.,",") %>% unlist() %>% trimws()
      # 
      tmp_genes = str_trim(str_split(tmp_anti$Targets, ";")[[1]])
      
      tmp_target = data.frame(cancer = Cancername , 
                              drug_name = dn,
                              target_genes = unique(tmp_anti$Targets),
                              critical_features = paste(colnames(gc_TCGA)[colnames(gc_TCGA) %in% c(single_genes %>% 
                                                                                                     filter(Genes %in% tmp_genes) %>% 
                                                                                                     pull(Pathway) ,
                                                                                                   link_genes_filtered_df %>% 
                                                                                                     filter(Genes  %in% tmp_genes) %>% 
                                                                                                     pull(Pathway))], collapse = ";")
      )
      
      tmp_long  = total_drug_screen %>% filter(DRUG_NAME == dn & cluster == "long")
      tmp_short  = total_drug_screen %>% filter(DRUG_NAME == dn & cluster == "short")
      tmp_for_drug = total_drug_screen %>% filter(DRUG_NAME == dn)
      anno_tmp = t.test(tmp_long$Z_SCORE, tmp_short$Z_SCORE)$p.value
      ...
      drug_target_features = rbind(drug_target_features, tmp_target)
      
      ggsave(filename = paste0(CancerType,"_",dn,"_gdsc_screening_short_spe.svg"), tmp_drug)
  ...
  sf_drug = unlist(str_split(drug_target_features$critical_features, ";"))
  
  total_drug_network = edges %>%
    as_tbl_graph() %N>% 
    mutate(sp_cf = case_when(name %in% sf_drug ~ 1,
                               .default = 0))  %E>%
    mutate(sp_cf = case_when(cf %in% sf_drug ~ 1, 
                               .default =0 ))%N>%
    mutate(color = case_when(sp_cf == 1  ~ "spe",
                             .default = "unspe")) %E>%
    mutate(color = case_when(sp_cf == 1 ~ "spe",
                             .default = "unspe")) %E>%
  ...
  ggsave(total_drug_network , filename = paste0(Cancername,"_specific_target_network_wo_common.svg"))
  
  write.csv(drug_target_features , paste0(Cancername, "_short_tolerant_drug_w_antic_target.csv"))
  
  # 
  # trimws(unlist(str_split(gsub(" \\([^\\)]+\\)","",unique(gdsc_w_cluster$PUTATIVE_TARGET)),"," )))[str_count(trimws(unlist(str_split(gsub(" \\([^\\)]+\\)","",unique(gdsc_w_cluster$PUTATIVE_TARGET)),"," )))) > 8]
  # 
  # 
  # unique( single_genes$Genes)
  # single_genes[which(grepl( "PIK3", single_genes$Genes )),]
  # single_genes[which(grepl( "MTOR", single_genes$Genes )),]
  # 
  # replace_gene_name = function(x) {
  #   x <- gsub("PI3Kalpha", "PIK3CA", x)
  #   x <- gsub("PI3Kbeta", "PIK3CB", x)
  #   x <- gsub("PI3Kgamma", "PIK3CG", x)
  #   x <- gsub("PI3Kdelta", "PIK3CD", x)
  #   
  #   return(x)
  # }
