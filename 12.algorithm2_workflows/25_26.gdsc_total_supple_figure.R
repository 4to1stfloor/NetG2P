...
for (num_CancerType in Cancerlist) {
  ...
  tmp_gd = gdsc_w_cluster_filt %>%
    select(Z_SCORE, cluster)

  total_gdsc = rbind(total_gdsc, tmp_gd)
}
...
total_long_gd = total_gdsc %>%
  filter(cluster == "long")

total_short_gd = total_gdsc %>%
  filter(cluster == "short")

anno = t.test(total_long_gd$Z_SCORE, total_short_gd$Z_SCORE)$p.value

total_gd = ggplot(total_gdsc , aes( x = cluster , y = Z_SCORE, fill=cluster)) + 
...
ggsave(filename = "total_treated_drug_IC50_zscore.svg", total_gd)
