...
GC.hyper.pathway.direct  <- function(CancerType, ppi.date, path.dat, path.file) {
  ...
  saveRDS(enr.mat, paste0(main.path_tc, "/phyper_enrichment_mut_", length(unique(path$Pathway)), "_", path.dat, "_GC_",ppi.date, ".rds"))
...
GC.hyper.path.net.direct  <- function(CancerType, ppi.date, path.dat, path.file) {
  ...
  saveRDS(dat, paste0(main.path_tc, "/phyper_enrichment_mut_", length(unique(path$Pathway)), "_", path.dat, "_net_GC_", ppi.date, ".rds"))
}
###########################################################################################################################################################################
...
cal.surv.each.p <- function(CancerType, ppi.date, path.dat, filt.genes.n) { 
  ...
  hyper.mat = readRDS(paste0(main.path_tc, "/phyper_enrichment_mut_", length(unique(path$Pathway)), "_", path.dat, "_GC_",ppi.date, ".rds"))
  #print(hyper.mat)
  # if (level != ""){
  ...
  if (CancerType %in% c("TCGA-COADREAD","TCGA-KIDNEY")) {
    ...
    int.id = intersect(substr(colnames(hyper.filt), 1, 16) , mut.filt$patient_id) 
    idx.id = match(int.id, substr(colnames(hyper.filt), 1,16))
  }
  ...
  if (level != "") {
    saveRDS(pval.com, paste0(main.path_tc, "/surv_pvals_54_mut_", path.dat, "_GC_", ppi.date, "_filt", filt.genes.n, ".rds"))
  } else {
    saveRDS(pval.com, paste0(main.path_tc, "/surv_pvals_54_mut_", path.dat, "_GC_", ppi.date, ".rds"))
  }
...
cal.surv.p <- function(CancerType, ppi.date, path.dat, level, filt.genes.n) { 
  ...
  hyper.mat = readRDS(paste0(main.path_tc, "/phyper_enrichment_mut_", length(unique(path$Pathway)), "_", path.dat, "_net_GC_", ppi.date, ".rds"))
  #print(hyper.mat)
  if (level != ""){
    sig.pair = dat$name[which(dat$n_genes > as.numeric(filt.genes.n))] 
    idx = match(sig.pair, rownames(hyper.mat))
  ...
  print(pval.com)
  if (level != "") {
    saveRDS(pval.com, paste0(main.path_tc, "/surv_pvals_54_mut_", path.dat, level, "_GC_", ppi.date, "_filt", filt.genes.n, ".rds"))
  } else {
    saveRDS(pval.com, paste0(main.path_tc,"/surv_pvals_54_mut_", path.dat, level, "_GC_", ppi.date, ".rds"))
  }
...
for (num_CancerType in Cancerlist) {
  ...
  cancermaf = readRDS(paste0(sce_path,num_CancerType,"/", CancerType,"_maf.rds"))
  ...
  # Filter MAF to exclude Nonsense_Mutation
  filtered_maf <- subsetMaf(maf = cancermaf, query = "Variant_Classification != 'Nonsense_Mutation'")
  
  cancer_filtered_mat = mutCountMatrix(filtered_maf)
 
  pval.mat = as.matrix(cancer_filtered_mat[intersect(hvgs, rownames(cancer_filtered_mat)),]) # expression of hvg!
