...
for (num_CancerType in Cancerlist) {
  ...
  if (length(long_gene) != 0 && length(short_gene) != 0 ) {
    ...
    if (length(unique(reducedTerms_short$parentTerm)) > 25) {
      ...
      count_short_head = names(tail(sort(table(reducedTerms_short_edit$parentTerm)), n= 8))
       
      total_short_imp = c()
      total_short_imp = unique(c(total_short_imp,  count_short_head))
      
    } else {
      reducedTerms_short_edit = reducedTerms_short
    
      count_short_head = names(tail(sort(table(reducedTerms_short_edit$parentTerm)), n= round(length(tail(sort(table(reducedTerms_short_edit$parentTerm)))) * 0.3)))
      total_short_imp = c()
      total_short_imp = unique(c(total_short_imp,  count_short_head))
    }
    ...
    if (length(unique(reducedTerms_long$parentTerm)) > 25) {
      ...
      count_long_head = names(tail(sort(table(reducedTerms_long_edit$parentTerm)), n= 8))
      
      total_long_imp = c()
      total_long_imp = unique(c(total_long_imp, count_long_head))
      
    } else {
      reducedTerms_long_edit = reducedTerms_long
      count_long_head = names(tail(sort(table(reducedTerms_long_edit$parentTerm)), n= round(length(tail(sort(table(reducedTerms_long_edit$parentTerm)))) * 0.3)))
    ...
    png(filename = paste0(CancerType,"_tree_short_0.7_wo_ttest_cut_top25_BP.png"),
    ...
    png(filename = paste0(CancerType,"_tree_long_0.7_wo_ttest_cut_top25_BP.png"),
        width = 35, height = 35,  units = "cm" ,pointsize = 12,
        bg = "white", res = 1200, family = "")
