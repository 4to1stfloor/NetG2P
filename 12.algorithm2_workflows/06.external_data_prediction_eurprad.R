...
eurprad_path ="C:/Users/user/Desktop/data/eurprad/"

CancerType_in = "eurprad"

# expretion matrix and convert form RPKM to TPM
eurprad_rpkm = read.table("C:/Users/user/Desktop/data/eurprad/data_mrna_seq_rpkm.txt", header = T)
eurprad_rpkm_filt = eurprad_rpkm
for (dupl in which(duplicated(eurprad_rpkm$Hugo_Symbol))) {
  print(dupl)
  dupl_gene = eurprad_rpkm[dupl,]$Hugo_Symbol
  tmp.df = eurprad_rpkm[which(eurprad_rpkm$Hugo_Symbol == dupl_gene),]
  tmp.df.2 = tmp.df[,c(-1,-2)]
  ...
  tmp.df.2= tmp.df.2 %>% relocate("Hugo_Symbol", .before = "CH79T")
  tmp.df.2= tmp.df.2 %>% relocate("Entrez_Gene_Id", .before = "CH79T")
  eurprad_rpkm_filt = eurprad_rpkm_filt[-which(eurprad_rpkm_filt$Hugo_Symbol == dupl_gene),]
  eurprad_rpkm_filt = rbind(eurprad_rpkm_filt,tmp.df.2)
  tmp.df.2 = NULL
  tmp.df =NULL
}

eurprad_rpkm_filt_tmp =eurprad_rpkm_filt[,c(-1,-2)]
eurprad_fpkm_filt_tmp = eurprad_rpkm_filt_tmp/2
fpkmToTpm <- function(fpkm)
{
  exp(log(fpkm) - log(sum(fpkm)) + log(1e6))
}

eurprad_tpm_filt =sapply(eurprad_fpkm_filt_tmp, fpkmToTpm)  
eurprad_tpm_filt = as.data.frame(eurprad_tpm_filt)
eurprad_tpm_filt$Hugo_Symbol = eurprad_rpkm_filt$Hugo_Symbol
eurprad_tpm_filt$Entrez_Gene_Id = eurprad_rpkm_filt$Entrez_Gene_Id
eurprad_tpm_filt= eurprad_tpm_filt %>% relocate("Hugo_Symbol", .before = "CH79T")
eurprad_tpm_filt= eurprad_tpm_filt %>% relocate("Entrez_Gene_Id", .before = "CH79T")
rownames(eurprad_tpm_filt) = eurprad_tpm_filt$Hugo_Symbol
eurprad_tpm_filt = eurprad_tpm_filt[,c(-1,-2)]

# mutation count matrix
eurprad_maf.data = read.maf(paste0(eurprad_path, "data_mutation.txt"))
mut.count.mat_eurprad = mutCountMatrix(eurprad_maf.data) 


saveRDS(eurprad_tpm_filt, file=paste0(eurprad_path, CancerType_in ,"_exp_TPM_mat_filt_log_gene_symbol.rds")) #60466 436 
saveRDS(mut.count.mat_eurprad, file=paste0(eurprad_path , CancerType_in ,"_mut_count_filt_data.rds"))
...
main.path = paste0(filepath, CancerType_in, "/")
