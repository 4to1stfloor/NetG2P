...
cptac_path ="C:/Users/user/Desktop/data/cptac/"

CancerType = "cptac"

data_cptac = read.csv(paste0(cptac_path, "coad_cptac_expdata.csv"), row.names = 1)
colnames(data_cptac) = substr(colnames(data_cptac),2,8)

# mutation count matrix
maf.data = read.maf(paste0(cptac_path, "data_mutation.txt"))
mut.count.mat_cptac = mutCountMatrix(maf.data) 


saveRDS(data_cptac, file=paste0(cptac_path, CancerType ,"_exp_TPM_mat_filt_log_gene_symbol.rds")) #60466 436 
saveRDS(mut.count.mat_cptac, file=paste0(cptac_path , CancerType ,"_mut_count_filt_data.rds"))
...
main.path = paste0(filepath, CancerType, "/")
...
} else if (dir.exists(paste0(rdpn_main.path, "03.RDPN/RDPN_",c(num.edge.multi),"X/")) && length(ck.rdpn.file) != 150) {
...
cal.surv.p <- function(CancerType, ppi.date, path.dat, level, filt.genes.n) { 
  ...
  cli = fread(paste0(main.path,"coad_cptac_2019_clinical_data.csv")) 
  cli
  cli_surv = cli[,
                 c("submitter_id",
                   "vital_status",
  ...
  # 
  cli_surv$overall_survival = as.numeric(cli_surv$overall_survival)
...
# Execute function 3
cal.surv.p(CancerType, ppi.date, path.dat, level) # this will generate "surv_pvals_54_KEGG_net_GC_20220117_filt5.rds" 
...
filepath ="C:/Users/user/Desktop/data/"
main.path_in = paste0(filepath , CancerType_in, "/")
surv.pvals_in = readRDS(paste0(filepath, CancerType_in,"/surv_pvals_54_KEGG_GC_20220117.rds"))
phyper_in = readRDS(paste0(filepath, CancerType_in, "/phyper_enrichment_54_KEGG_GC_20220117.rds"))

mut.mat_in = readRDS(paste0(filepath , CancerType_in,"/", CancerType_in,"_mut_count_filt_data.rds"))
...
cli_cptac = fread(paste0(main.path_in,"coad_cptac_2019_clinical_data.csv")) 
cli_cptac
cli_cptac_surv = cli_cptac[,
               c("submitter_id",
                 "vital_status",
                 "overall_survival")]
...
# cli_surv = cli_surv[!is.na(cli_surv$vital_status),]
# 
cli_cptac_surv$status = NA
cli_cptac_surv$status[which(cli_cptac_surv$vital_status == "Dead")] = 1
cli_cptac_surv$status[which(cli_cptac_surv$vital_status == "Alive")] = 0
# 
cli_cptac_surv$overall_survival = as.numeric(cli_cptac_surv$overall_survival)
cli_cptac_surv$status = as.numeric(cli_cptac_surv$status)
...
surv_in = surv.pvals_in
surv_in$hallmark = rownames(surv_in)
surv.sig_in = surv_in$hallmark[which(surv_in[,1] < 0.05)]
surv.sig_in
surv.sig
##########################################################
...
for (n in 1:length(surv.sig_in)) {
  phyper.each_in = phyper.filt_in[which(rownames(phyper.filt_in) == surv.sig_in[n]), ]
  phyper.dat_in = as.data.frame(cbind(id = colnames(phyper.each_in), pvals = as.numeric(phyper.each_in)))
  
  if (n == 1) {
    phyper.dat.2_in= as.numeric(phyper.dat_in$pvals)
  } else {
    phyper.dat.2_in = rbind(phyper.dat.2_in, as.numeric(phyper.dat_in$pvals))
  }
}

phyper.dat.2_in
#phyper.dat.2 = t(as.matrix(phyper.dat.2))
rownames(phyper.dat.2_in) = surv.sig_in
colnames(phyper.dat.2_in) = colnames(phyper.filt_in)

#### for reference surv.sig

for (n in 1:length(surv.sig)) {
  phyper.each_in = phyper.filt_in[which(rownames(phyper.filt_in) == surv.sig[n]), ]
  phyper.dat_in = as.data.frame(cbind(id = names(phyper.each_in), pvals = as.numeric(phyper.each_in)))
...
phyper.dat.2_in
#phyper.dat.2 = t(as.matrix(phyper.dat.2))
rownames(phyper.dat.2_in) = surv.sig
colnames(phyper.dat.2_in) = colnames(phyper.filt_in)
...
colnames(phyper.filt_in_t_log) = substr(colnames(phyper.filt_in_t_log), 1,12)
...
for (all_sub in cli_cptac_surv$submitter_id) {
...
df.phyper.merge_in_log_all = as.data.frame(merge(cli_cptac_surv , df.phyper.filt_in_t_log_num, by = "submitter_id"))
...
df.phyper.2_in = as.data.frame(phyper.filt_in)

df.phyper.all_in= data.frame(matrix(nrow = length(rownames(phyper.dat.2_in))))
for (all_sub in cli_cptac_surv$submitter_id) {
  if (all_sub %in% colnames(phyper.dat.2_in)) {
    df.phyper.all_in = cbind(df.phyper.all_in,df.phyper.2_in[all_sub])
  }
  
}  
df.phyper.all_in= df.phyper.all_in[,-1]
df.phyper.all_in_t = t(df.phyper.all_in)
df.phyper.all_in_t = as.data.frame(df.phyper.all_in_t)
df.phyper.all_in_t$submitter_id = rownames(df.phyper.all_in_t)
df.phyper.all_in_t

df.phyper.merge_in = as.data.frame(merge(cli_cptac_surv , df.phyper.all_in_t, by = "submitter_id"))
rownames(df.phyper.merge_in) = df.phyper.merge_in$submitter_id
...
df.phyper.merge_in_log_all_filt$Alive = ifelse(df.phyper.merge_in_log_all_filt$vital_status == "Alive",
                               1,0)
df.phyper.merge_in_log_all_filt$Dead = ifelse(df.phyper.merge_in_log_all_filt$vital_status == "Dead",
                              1,0)
df.phyper.merge_log_in3 = df.phyper.merge_in_log_all_filt[,c(-1,-2,-3,-4)]
df.phyper.merge_log_in3
df.phyper.merge_vital_log_in2= df.phyper.merge_log_in3

df.phyper.merge_vital_log_in2

###
## for neuralnet 
####

df.phyper.merge_in_log_all_status = df.phyper.merge_in_log_all_filt[,c(-1,-2,-3)]
df.phyper.merge_in_log_all_status= df.phyper.merge_in_log_all_status %>% relocate("status", .after = "P54")
df.phyper.merge_in_log_all_status = df.phyper.merge_in_log_all_status[,-c(length(colnames(df.phyper.merge_in_log_all_status)),length(colnames(df.phyper.merge_in_log_all_status))-1)]
colnames(df.phyper.merge_log_all_status)= gsub("[[:punct:]]", "", colnames(df.phyper.merge_log_all_status))
...
# validate
library(dplyr)
predic_in_log_df = as.data.frame(predict(cli_log_nnv, df.phyper.merge_vital_log_in2))
results_cptac_alive <- data.frame(actual_alive = df.phyper.merge_vital_log_in2$Alive, prediction_alive = predic_in_log_df$Alive)
results_cptac_Dead <- data.frame(actual_dead = df.phyper.merge_vital_log_in2$Dead, prediction_dead = predic_in_log_df$Dead)


roundedresults_cptac_alive<-sapply(results_cptac_alive,round,digits=0)
roundedresults_cptac_dead<-sapply(results_cptac_Dead,round,digits=0)

roundedresultsdf_cptac_alive=data.frame(roundedresults_cptac_alive)
roundedresultsdf_cptac_dead=data.frame(roundedresults_cptac_dead)

roundedresultsdf_cptac_alive = roundedresultsdf_cptac_alive %>% mutate(
...
roundedresultsdf_cptac_dead = roundedresultsdf_cptac_dead %>% mutate(
...
rownames(roundedresultsdf_cptac_alive) = rownames(predic_in_log_df)
rownames(roundedresultsdf_cptac_dead) = rownames(predic_in_log_df)

attach(roundedresultsdf_cptac_alive)
result_cptac.alive.table= table(actual_alive,prediction_alive)
write.csv(result_cptac.alive.table, paste0(main.path_in,"results_alive_log_table_for_ann_cli.csv"))

attach(roundedresultsdf_cptac_dead)
result_cptac.dead.table= table(actual_dead,prediction_dead)
write.csv(result_cptac.dead.table, paste0(main.path_in,"results_dead_log_table_for_ann_cli.csv"))

predic_df$actual_alive = df.phyper.merge_vital_in2$Alive
predic_df$actual_dead = df.phyper.merge_vital_in2$Dead

####################### draw survival plot
predic_in_round_log_df = sapply(predic_in_log_df,round,digits=0)
predic_in_round_log_df = as.data.frame(predic_in_round_log_df)

predic_in_round_log_df = predic_in_round_log_df %>% mutate(
  Alive = ifelse( Alive <0 , 0 ,Alive),
  Alive = ifelse( Alive >1 , 1 ,Alive),
  Dead = ifelse(Dead <0 , 0 , Dead ),
  Dead = ifelse(Dead >1 , 1 , Dead )
)
rownames(predic_in_round_log_df) = rownames(predic_in_log_df)

if (sum(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead ) !=0 ) {
  if (predic_in_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),]$Alive > predic_in_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),]$Dead) {
    predic_in_round_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),1] = 1
  } else if (predic_in_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),]$Alive < predic_in_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),]$Dead) {
    predic_in_round_log_df[which(predic_in_round_log_df$Alive == predic_in_round_log_df$Dead),2] = 1
  }
}

cli_cptac_surv_filt = data.frame()
for (sub_pat_id in rownames(roundedresultsdf_cptac_alive)) {
  if (sub_pat_id %in% cli_cptac_surv$submitter_id) {
    cli_cptac_tmp = as.data.frame(cli_cptac_surv[which(cli_cptac_surv$submitter_id == sub_pat_id),])
    # print(cli_cptac_tmp)
    cli_cptac_surv_filt = rbind(cli_cptac_surv_filt,cli_cptac_tmp)
    
  }
}

rownames(cli_cptac_surv_filt) = cli_cptac_surv_filt$submitter_id
cli_cptac_surv_filt$predicted = NA
for (sub_pat_id2 in rownames(roundedresultsdf_cptac_alive)) {
  if (which(predic_in_round_log_df[sub_pat_id2,] == 1) == 1) {
    cli_cptac_surv_filt[sub_pat_id2,]$predicted = "Alive"
  } else if (which(predic_in_round_log_df[sub_pat_id2,] == 1) == 2) {
    cli_cptac_surv_filt[sub_pat_id2,]$predicted = "Dead"
  }
}

fit_cptac = survfit(Surv(overall_survival, status) ~ predicted, data = cli_cptac_surv_filt)

ggsurvplot(fit_cptac, data = cli_cptac_surv_filt, risk.table = TRUE,
