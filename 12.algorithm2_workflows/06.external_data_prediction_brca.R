...
expression_filt = as.data.frame(expression_filt)
rownames(expression_filt) = tmp_rowname
expression_filt

expression_filt = expression_filt[,-which(colSums(is.na(expression_filt)) != 0)]

# mutation count matrix
ex_maf.data = read.maf(paste0(main.path, "data_mutations.txt"))
mut.count.mat_ex = mutCountMatrix(ex_maf.data) 

expression_filt_final= expression_filt[,intersect(colnames(mut.count.mat_ex) ,colnames(expression_filt ))]
mut.count.mat_ex_final = mut.count.mat_ex[,intersect(colnames(mut.count.mat_ex) ,colnames(expression_filt ))]

saveRDS(expression_filt_final, file=paste0(main.path, CancerType_in ,"_exp_TPM_mat_filt_log_gene_symbol.rds")) #60466 436 
saveRDS(mut.count.mat_ex_final, file=paste0(main.path,CancerType_in,"_mut_count_filt_data.rds"))
...
cal.surv.p <- function(CancerType, ppi.date, path.dat, level, filt.genes.n) { 
  ...
  cli_surv$submitter_id = cli_surv$`Patient ID`
