  ...
  if (length(long_gene) != 0 && length(short_gene) != 0 ) {
    ...
    if (length(unique(reducedTerms_short$parentTerm)) > 25) {
      reducedTerms_short_edit = reducedTerms_short[which(reducedTerms_short$parentTerm %in% 
                                                           unique(reducedTerms_short[order(reducedTerms_short$score, decreasing = T),]$parentTerm)[1:25]),]
      
      reducedTerms_short_edit = reducedTerms_short_edit %>% mutate(total_short_imp = case_when(parentTerm %in% 
                                                                                                reducedTerms_short_edit$parentTerm[grep(paste(metabolism, collapse = "|"), reducedTerms_short_edit$parentTerm)] ~ "meta",
                                                                                             parentTerm %in% 
                                                                                               reducedTerms_short_edit$parentTerm[grep(paste(immune, collapse = "|"), reducedTerms_short_edit$parentTerm)] ~ "immune",
                                                                                             parentTerm %in% 
                                                                                               reducedTerms_short_edit$parentTerm[grep(paste(metabolism, collapse = "|"), reducedTerms_short_edit$parentTerm)] &
                                                                                               parentTerm %in% 
                                                                                               reducedTerms_short_edit$parentTerm[grep(paste(immune, collapse = "|"), reducedTerms_short_edit$parentTerm)] ~ "dual", 
                                                                                             .default = "not_include"
      ))
      
      total_short_imp = c()
      total_short_imp = unique(reducedTerms_short_edit$total_short_imp)
    ...
    if (length(unique(reducedTerms_long$parentTerm)) > 25) {
      reducedTerms_long_edit = reducedTerms_long[which(reducedTerms_long$parentTerm %in% 
                                                         unique(reducedTerms_long[order(reducedTerms_long$score, decreasing = T),]$parentTerm)[1:25]),]
      
      reducedTerms_long_edit = reducedTerms_long_edit %>% mutate(total_long_imp = case_when(parentTerm %in% 
                                                                                         reducedTerms_long_edit$parentTerm[grep(paste(metabolism, collapse = "|"), reducedTerms_long_edit$parentTerm)] ~ "meta",
                                                                                         parentTerm %in% 
                                                                                         reducedTerms_long_edit$parentTerm[grep(paste(immune, collapse = "|"), reducedTerms_long_edit$parentTerm)] ~ "immune",
                                                                                       parentTerm %in% 
                                                                                         reducedTerms_long_edit$parentTerm[grep(paste(metabolism, collapse = "|"), reducedTerms_long_edit$parentTerm)] &
                                                                                       parentTerm %in% 
                                                                                         reducedTerms_long_edit$parentTerm[grep(paste(immune, collapse = "|"), reducedTerms_long_edit$parentTerm)] ~ "dual", 
                                                                                       .default = "not_include"
                                                                                       ))
 
      total_long_imp = c()
      total_long_imp = unique(reducedTerms_long_edit$total_long_imp)
    ...
    # reducedTerms_short_add = reducedTerms_short_edit %>% 
    #   mutate(color = case_when( parentTerm %in% total_short_imp ~ "#CC0000",
    #                             .default = "#999999")) %>% 
    #   mutate(subgroup_font_size = case_when( parentTerm %in% total_short_imp ~ 25, .default = 10))%>% 
    #   mutate(subgroup_font_fontface = case_when( parentTerm %in% total_short_imp ~ "bold",.default = "plain"))
    # 
    # reducedTerms_long_add = reducedTerms_long_edit %>% 
    #   mutate(color = case_when( parentTerm %in% total_long_imp ~ "#339900",
    #                             .default = "#999999")) %>% 
    #   mutate(subgroup_font_size = case_when( parentTerm %in% total_long_imp ~ 25, .default = 10))%>% 
    #   mutate(subgroup_font_fontface = case_when( parentTerm %in% total_long_imp ~ "bold",.default = "plain"))
    # unique(reducedTerms_short_edit$total_short_imp)
    # unique(reducedTerms_long_edit$total_long_imp)
    
    reducedTerms_short_add = reducedTerms_short_edit %>% 
      mutate(color = case_when( total_short_imp == "meta" ~ "#CC0000",
                                total_short_imp == "immune" ~ "#339900",
                                .default = "#999999")) %>% 
      mutate(subgroup_font_size = case_when( total_short_imp == "meta" ~ 25,
                                             total_short_imp == "immune" ~ 25,
                                             .default = 10))%>% 
      mutate(subgroup_font_fontface = case_when( total_short_imp == "meta" ~ "bold",
                                                 total_short_imp == "immune" ~ "bold",
                                                 .default = "plain"))
    
    reducedTerms_long_add = reducedTerms_long_edit %>% 
      mutate(color = case_when( total_long_imp == "meta" ~ "#CC0000",
                                total_long_imp == "immune" ~ "#339900",
                                .default = "#999999")) %>% 
      mutate(subgroup_font_size = case_when( total_long_imp == "meta" ~ 25,
                                             total_long_imp == "immune" ~ 25,
                                             .default = 10))%>% 
      mutate(subgroup_font_fontface = case_when( total_long_imp == "meta" ~ "bold",
                                                 total_long_imp == "immune" ~ "bold",
                                                 .default = "plain"))
    
    # fiq
    
    png(filename = paste0(CancerType,"_tree_short_0.7_wo_ttest_immune_meta.png"),
        width = 35, height = 35,  units = "cm" ,pointsize = 12,
        bg = "white", res = 1200, family = "")
    
    tree_mat_short =  ggplot(reducedTerms_short_add, aes(area = score, fill = color, 
                                                         label = term, # country
      ...
      geom_treemap_subgroup_text(place = "centre", colour = "black",
                                 reflow = T,
                                 fontface = reducedTerms_short_add$subgroup_font_fontface,
                                 # min.size = 0
                                 start = "topleft",
                                 grow = F,
                                 size= reducedTerms_short_add$subgroup_font_size) +
    ...
    png(filename = paste0(CancerType,"_tree_long_0.7_wo_ttest_immune_meta.png"),
        width = 35, height = 35,  units = "cm" ,pointsize = 12,
        bg = "white", res = 1200, family = "")
    
    tree_mat_long =  ggplot(reducedTerms_long_add, aes(area = score, fill = color, 
                                                       label = term, # country
                                                       subgroup = parentTerm)) + # continent
      scale_fill_identity() +
      # 1. Draw country borders and fill colors
      ...
      geom_treemap_subgroup_text(place = "centre", colour = "black",
                                 reflow = T,
                                 fontface = reducedTerms_long_add$subgroup_font_fontface,
                                 # min.size = 0
                                 start = "topleft",
                                 grow = F,
                                 size= reducedTerms_long_add$subgroup_font_size) +
      # 4. Print country text
      geom_treemap_text(colour = "#FFFFFFDD", 
                        place = "centre",
                        alpha = 0.5,
