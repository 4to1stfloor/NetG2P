...
cal.surv.each.p <- function(CancerType, ppi.date, path.dat, filt.genes.n) { 
  ...
  saveRDS(pval.com, paste0(main.path, "/surv_pvals_54_", path.dat, "_GC_", ppi.date, "_filt", filt.genes.n, ".rds"))
...
for (num_CancerType in Cancerlist) {
  ...
  if (file.exists(paste0(main.path, "surv_pvals_54_", path.dat, level, "_GC_", ppi.date, "_filt", filt.genes.n, ".rds"))) {
    next
  }
  # Execute function 1
