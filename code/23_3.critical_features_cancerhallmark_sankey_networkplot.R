...
for (num_CancerType in Cancerlist) {
  ...
  for (tmp_source in unique(tmp_cancer_links$source )) {
    source_num = as.numeric(rownames(tmp_cancer_nodes)[which(tmp_cancer_nodes$name == tmp_source)])
    tmp_cancer_links[which(tmp_cancer_links$source == tmp_source),]$source = source_num
  
    }

  for (tmp_target in unique(tmp_cancer_links$target )) {
    target_num = as.numeric(rownames(tmp_cancer_nodes)[which(tmp_cancer_nodes$name == tmp_target)])
    tmp_cancer_links[which(tmp_cancer_links$target == tmp_target),]$target = target_num
    
  }
  
  tmp_cancer_links$source = as.numeric(tmp_cancer_links$source) -1
  tmp_cancer_links$target = as.numeric(tmp_cancer_links$target) -1
  
  sankeyNetwork_li = list(nodes = tmp_cancer_nodes , links = tmp_cancer_links)
  ...
  sankeyNetwork(Links = sankeyNetwork_li$links, Nodes = sankeyNetwork_li$nodes, Source = "source",
    ...
    saveNetwork(file = paste0(CancerType,"_sanketnet_ch_bf.html"), FALSE)
