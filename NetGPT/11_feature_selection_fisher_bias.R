...
for (num_CancerType in Cancerlist) {
  ...
  if (sum(is.infinite(feature_pval_specific$alive_to_dead_ratio)) == TRUE) {
  ...
  data_tc_ori = cbind(data_tc_ori[,speci_features], vitalstatus = data_tc_ori$vitalstatus)
  ...
  train_tc$status = ifelse(train_tc$vitalstatus == "Alive", 0 , 1)
  train_tc = train_tc[,-(ncol(train_tc)-1)]
  valid_tc$status = ifelse(valid_tc$vitalstatus == "Alive", 0 , 1)
  valid_tc = valid_tc[,-(ncol(valid_tc)-1)]
  test_tc$status = ifelse(test_tc$vitalstatus == "Alive", 0 , 1)
  test_tc = test_tc[,-(ncol(test_tc)-1)]
