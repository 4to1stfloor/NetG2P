...
for (num_CancerType in Cancerlist) {
  ...
  #   A. for each pathway
  if (type == "each") {
    surv.pvals = readRDS(paste0(main.path_tc,"/surv_pvals_54_exp_KEGG_GC_20220117.rds"))
    phyper = readRDS(paste0(main.path_tc, "/phyper_enrichment_exp_54_KEGG_GC_20220117.rds"))
  } else if (type == "link") {
    #   B. for pathwaylink
    surv.pvals = readRDS(paste0(main.path_tc,"/surv_pvals_54_exp_KEGG_net_GC_20220117.rds"))
    phyper = readRDS(paste0(main.path_tc, "/phyper_enrichment_exp_54_KEGG_net_GC_20220117.rds"))
  }
  ...
  if (type == "link") {
    ...
    saveRDS(df.phyper.merge_vital_log_all2, file = paste0(main.path_tc,"/",CancerType,"_exp_pathwaylink_all_log.rds"))
    
  } else if (type == "each") {
    df.phyper.merge_vital_log_all2= df.phyper.merge_vital_log_all2 %>% relocate("vitalstatus", .after = "P54")
    saveRDS(df.phyper.merge_vital_log_all2, file = paste0(main.path_tc,"/",CancerType,"_exp_pathwayeach_all_log.rds"))
  }
