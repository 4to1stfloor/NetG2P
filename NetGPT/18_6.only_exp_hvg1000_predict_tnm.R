...
for (num_CancerType in Cancerlist) {
  ...
  sce_exp_filt2_df = sce_exp_filt_t_df_filt[,substr(colnames(sce_exp_filt_t_df_filt),1,12) %in% cli_surv_filtered$submitter_id]
  
  if (ncol(sce_exp_filt_t_df_filt) != length(cli_surv_filtered$submitter_id) ) {
    cli_surv_filt  = cli_surv_filtered[cli_surv_filtered$submitter_id %in% substr(colnames(sce_exp_filt2_df),1,12) ,]
  }
  
  if (CancerType %in% c("TCGA-COADREAD","TCGA-KIDNEY")) {
    
    sce_exp_filt2_df = sce_exp_filt_t_df_filt[,intersect(substr(colnames(sce_exp_filt_t_df_filt),1,12) , cli_surv_filtered$submitter_id)]
    
  } 
  
  if (all.equal(ncol(sce_exp_filt2_df),  length(cli_surv_filt$submitter_id) )) {
    sce_exp_filt2_df = as.data.frame(t(sce_exp_filt2_df))
    sce_exp_filt2_df$pat_t = cli_surv_filt$ajcc_pathologic_t 
    sce_exp_filt2_df$pat_n = cli_surv_filt$ajcc_pathologic_n 
    sce_exp_filt2_df$pat_m = cli_surv_filt$ajcc_pathologic_m 
  }
  ...
  tmp_tnm = sce_count_log_with_sym_replaced[,c("pat_t","pat_n","pat_m")]
  sce_exp_filt3_df = sce_count_log_with_sym_replaced[,which(!colnames(sce_count_log_with_sym_replaced) %in% c("pat_t","pat_n","pat_m"))]
  sce_exp_filt3_df[sce_exp_filt3_df > 4] <- 4
  sce_exp_filt3_df[sce_exp_filt3_df < -4] <- -4
  sce_exp_filt4_df = cbind(sce_exp_filt3_df,tmp_tnm)
  
  sce_exp_filt4_df = sce_exp_filt4_df[order(sce_exp_filt4_df$pat_t),]
  annotation_df <- data.frame(pat_t = sce_exp_filt4_df$pat_t,
                              pat_n = sce_exp_filt4_df$pat_n,
                              pat_m = sce_exp_filt4_df$pat_m)
  rownames(annotation_df) <- rownames(sce_exp_filt4_df)
  
  # Create a named color vector for the unique values of vital_status
  
  num_pat_t = brewer.pal(length(unique(sce_exp_filt4_df$pat_t)), "RdGy")
  col_pat_t = setNames(num_pat_t, unique(sce_exp_filt4_df$pat_t))
 
  num_pat_n = brewer.pal(length(unique(sce_exp_filt4_df$pat_n)), "Spectral")
  col_pat_n = setNames(num_pat_n, unique(sce_exp_filt4_df$pat_n))
  
  num_pat_m = brewer.pal(length(unique(sce_exp_filt4_df$pat_m)), "Set3")
  col_pat_m = setNames(num_pat_m, unique(sce_exp_filt4_df$pat_m))
  
  tnm_colors = list(pat_t = col_pat_t , pat_n = col_pat_n, pat_m = col_pat_m)
  
  png(filename = paste0(CancerType,"_exp_colum_clusterT_tnm.png"),
      width = 25, height = 25,  units = "cm" ,pointsize = 12,
      bg = "white", res = 1200, family = "")
  
  tmp = pheatmap::pheatmap(as.matrix(t(sce_exp_filt4_df[,which(!colnames(sce_exp_filt4_df) %in% c("pat_t","pat_n","pat_m"))])),
  ...
  png(filename = paste0(CancerType,"_exp_column_clusterF_tnm.png"),
      width = 25, height = 25,  units = "cm" ,pointsize = 12,
      bg = "white", res = 1200, family = "")
  
  tmp2 = pheatmap::pheatmap(as.matrix(t(sce_exp_filt4_df[,which(!colnames(sce_exp_filt4_df) %in% c("pat_t","pat_n","pat_m"))])),
                           annotation_col = annotation_df,
                           annotation_colors = tnm_colors,
                           cluster_cols = F)
  print(tmp2)
  ...
  png(filename = paste0(CancerType,"_exp_cluster_t_complex.png"),
      width = 25, height = 25,  units = "cm" ,pointsize = 12,
      bg = "white", res = 1200, family = "")
  tmp3 = ComplexHeatmap::pheatmap(as.matrix(t(sce_exp_filt4_df[,which(!colnames(sce_exp_filt4_df) %in% c("pat_t","pat_n","pat_m"))])),
  ...
  png(filename = paste0(CancerType,"_exp_cluster_n_complex.png"),
      width = 25, height = 25,  units = "cm" ,pointsize = 12,
      bg = "white", res = 1200, family = "")
  
  tmp4 = ComplexHeatmap::pheatmap(as.matrix(t(sce_exp_filt4_df[,which(!colnames(sce_exp_filt4_df) %in% c("pat_t","pat_n","pat_m"))])),
  ...
  png(filename = paste0(CancerType,"_exp_cluster_m_complex.png"),
      width = 25, height = 25,  units = "cm" ,pointsize = 12,
      bg = "white", res = 1200, family = "")
  tmp5 = ComplexHeatmap::pheatmap(as.matrix(t(sce_exp_filt4_df[,which(!colnames(sce_exp_filt4_df) %in% c("pat_t","pat_n","pat_m"))])),
  ...
  remove(tmp,tmp2,tmp3,tmp4,tmp5,sce_exp_filt4_df,sce_exp_filt3_df,sce_exp_filt2_df,cli_surv_filt,sce_exp_filt_t_df_filt,cli_surv, sce_for_hvgs, sce)
